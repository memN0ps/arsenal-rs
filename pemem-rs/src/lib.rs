...
use core::{arch::asm, mem::size_of, slice::from_raw_parts};
use ntapi::{
    ntpebteb::{PPEB, PTEB},
    ...
    winapi::um::{
        ...
        winnt::{
            ...
            IMAGE_DIRECTORY_ENTRY_IMPORT, IMAGE_DOS_SIGNATURE, IMAGE_IMPORT_DESCRIPTOR,
            IMAGE_NT_SIGNATURE, IMAGE_ORDINAL64, IMAGE_REL_BASED_DIR64, IMAGE_REL_BASED_HIGHLOW,
            IMAGE_SNAP_BY_ORDINAL64, PIMAGE_BASE_RELOCATION, PIMAGE_DOS_HEADER,
            PIMAGE_EXPORT_DIRECTORY, PIMAGE_IMPORT_BY_NAME, PIMAGE_IMPORT_DESCRIPTOR,
            PIMAGE_NT_HEADERS64, PIMAGE_SECTION_HEADER, PIMAGE_THUNK_DATA64,
        },
...
pub unsafe fn rebase_image(module_base: *mut u8) -> Option<bool> 
{
    ...
    let mut base_relocation = (module_base as usize + (*nt_headers).OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC as usize].VirtualAddress as usize) as PIMAGE_BASE_RELOCATION;
...
pub unsafe fn resolve_imports(module_base: *mut u8) -> Option<bool> 
{
    ...
    let mut import_directory = (module_base as usize + (*nt_headers).OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT as usize].VirtualAddress as usize) as PIMAGE_IMPORT_DESCRIPTOR;
...
mod tests {
    ...
    {
        assert_eq!(dbj2_hash("ABCDEFGHIJKLMNOPQRSTUVWXYZ".as_bytes()), 0xd6989ec4);
        assert_eq!(dbj2_hash("abcdefghijklmnopqrstuvwxyz".as_bytes()), 0xd6989ec4);
        assert_eq!(dbj2_hash("0123456789".as_bytes()), 0x3b5a4652);
    }
    ...
    fn test_pattern_scan()
    {
        ...
        let kernelbase_slice = unsafe { core::slice::from_raw_parts(kernelbase as _, image_size as _) };
